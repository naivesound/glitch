...
public:
  ...
  std::string asString() const {
    ...
    if (addr().sa_family) {
      ...
      int err = getnameinfo(&addr_.sa, sizeof addr_, hostname, sizeof hostname,
...
struct UdpSocket {
  ...
  SockAddr remote_addr; /* initialised for connected sockets. Also updated for
                           bound sockets after each datagram received */
  ...
  enum {
    ...
    OPTION_FORCE_IPV4 = 1,
    OPTION_FORCE_IPV6 = 2,
    OPTION_DEFAULT = OPTION_FORCE_IPV4 // according to liblo's README, using
                                       // ipv6 sockets causes issues with other
                                       // non-ipv6 enabled osc software
  };

  /** open the socket and bind it to a port. Use this when you want to read
      incoming data on the specified port, using the function
     receiveNextDatagram.
  */
  ...
  /** open the socket, and prepare for sending datagrams to the specified
   * host:port */
  ...
  /** wait for the next datagram to arrive on our bound socket. Return
      ...
      The datagram is available with the getDatagramData() / getDatagramSize()
     functions,
...
// maybe here we should differentiate EAGAIN/EINTR/EWOULDBLOCK from real errors
    ...
    if (nread > (int)buffer.size()) {
      /* no luck... a large datagram arrived and we truncated it.. now it is too
       * late */
      buffer.clear();
    } else {
      buffer.resize(nread);
  ...
  bool sendPacketTo(const void *ptr, size_t sz, SockAddr &addr) {
    ...
    do {
      int res;
      if (isBound()) {
        res = sendto(handle, (const char *)ptr, (int)sz, 0, &addr.addr(),
                     (int)addr.actualLen());
      } else {
      ...
      if (res == -1 && WSAGetLastError() == WSAEINTR)
        ...
        sent = res;
#else
      // if (res == -1) cerr << "sendto handle=" << handle << ", res:" << res <<
      // ", sz=" << sz << ", errno=" << errno << " " << strerror(errno) << "\n";
...
private:
  ...
  bool openSocket(const std::string &hostname, const std::string &port,
    ...
    if (options == OPTION_FORCE_IPV4)
      ...
      hints.ai_family =
          AF_UNSPEC; /* Allow IPv4 or IPv6 -- in case of problem, try with
                        AF_INET ...*/
    hints.ai_socktype = SOCK_DGRAM; /* Datagram socket */
    hints.ai_flags =
        (binding
             ? AI_PASSIVE
             : 0); /* AI_PASSIVE means socket address is intended for bind */

    int err = 0;

    err = getaddrinfo(binding ? 0 : hostname.c_str(),
